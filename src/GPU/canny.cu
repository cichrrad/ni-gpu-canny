...
// TODO -- SPLIT TO PROCESS BY CHANNEL
// ?TODO -- merge (atomic add / separate kernel)

__global__ void kernelGrayscale(const uchar3 *input, unsigned char *output,
...
// ===========================
__global__ void kernelGaussianBlur(const unsigned char *input,
...
// ===========================
__global__ void kernelSobel(const unsigned char *input, float *magnitude,
...
// ===========================
__global__ void kernelNonMaxSuppression(const float *magnitude,
                                        const float *direction, float *output,
                                        int width, int height) {
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  if (row >= 1 && row < height - 1 && col >= 1 && col < width - 1) {
    ...
    if (d == 0.0f) { // horizontal edge; compare left/right
      n1 = magnitude[row * width + (col - 1)];
      n2 = magnitude[row * width + (col + 1)];
    } else if (d == 90.0f) { // vertical edge; compare top/bottom
      n1 = magnitude[(row - 1) * width + col];
      n2 = magnitude[(row + 1) * width + col];
    } else if (d == 45.0f) { // diagonal top-left and bottom-right
      n1 = magnitude[(row - 1) * width + (col - 1)];
      n2 = magnitude[(row + 1) * width + (col + 1)];
    } else if (d == 135.0f) { // diagonal top-right and bottom-left
      n1 = magnitude[(row - 1) * width + (col + 1)];
      n2 = magnitude[(row + 1) * width + (col - 1)];
    }
...
// ===========================
__global__ void kernelDoubleThreshold(const float *input, unsigned char *output,
...
__global__ void kernelHysteresis(unsigned char *image, int width, int height) {
  ...
  if (row >= 1 && row < height - 1 && col >= 1 && col < width - 1) {
    int idx = row * width + col;
    if (image[idx] ==
        255) { // if current pixel is a strong edge, check neighbors
      for (int dx = -1; dx <= 1; dx++) {
        for (int dy = -1; dy <= 1; dy++) {
          int nIdx = (row + dx) * width + (col + dy);
          if (image[nIdx] == 128) { // if weak edge, promote it
            image[nIdx] = 255;
          }
        }
      }
...
int main(int argc, char **argv) {
  ...
  cudaMemcpy(d_src, src.ptr<uchar3>(), numPixels * sizeof(uchar3),
  ...
  double grayscaleTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t1_stop - t1_start)
          .count();
  // TIMED END
  ...
  double blurTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t2_stop - t2_start)
          .count();
  // TIMED END
  ...
  kernelSobel<<<gridDim, blockDim>>>(d_blur, d_gradMag, d_gradDir, width,
                                     height);
  cudaDeviceSynchronize();

  auto t3_stop = std::chrono::high_resolution_clock::now();
  double sobelTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t3_stop - t3_start)
          .count();
  // TIMED END
  ...
  kernelNonMaxSuppression<<<gridDim, blockDim>>>(d_gradMag, d_gradDir, d_nms,
                                                 width, height);
  cudaDeviceSynchronize();

  auto t4_stop = std::chrono::high_resolution_clock::now();
  double nmsTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t4_stop - t4_start)
          .count();
  // TIMED END
  ...
  kernelDoubleThreshold<<<blocks, threadsPerBlock>>>(d_nms, d_dt, width, height,
                                                     t_low, t_high);
  cudaDeviceSynchronize();

  auto t5_stop = std::chrono::high_resolution_clock::now();
  double dtTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t5_stop - t5_start)
          .count();
  // TIMED END
  ...
  double hysteresisTime =
      std::chrono::duration_cast<std::chrono::microseconds>(t6_stop - t6_start)
          .count();
  // TIMED END

  auto total =
      grayscaleTime + blurTime + sobelTime + nmsTime + dtTime + hysteresisTime;

  // Get file size using filesystem API
  std::uintmax_t filesize = std::filesystem::file_size(inputPath);

  // Print results in a key=value format for easy parsing
  std::cout << "FILE = " << filename << "\n";
  std::cout << "SIZE = " << filesize << "\n";
  std::cout << "GStime = " << grayscaleTime << "\n";
  std::cout << "GBtime = " << blurTime << "\n";
  std::cout << "STtime = " << sobelTime << "\n";
  std::cout << "NMS = " << nmsTime << "\n";
  std::cout << "DTtime = " << dtTime << "\n";
  std::cout << "Hystime = " << hysteresisTime << "\n";
  std::cout << "TOTAL = " << total << "\n";

  // Copy final result from device back to host (final edge image is in d_dt)
  cv::Mat result(height, width, CV_8UC1);
  cudaMemcpy(result.ptr<unsigned char>(), d_dt,
